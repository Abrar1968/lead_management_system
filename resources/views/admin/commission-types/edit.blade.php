<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-bold text-gray-900">Edit: {{ $commissionType->name }}</h2>
                    <p class="text-sm text-gray-500">Update commission structure settings</p>
                </div>
            </div>
    ...
    <div class="max-w-3xl">
        <div class="rounded-2xl bg-white shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.commission-types.update', $commissionType) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $commissionType->name) }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        ...
                        placeholder="Describe when this commission type is applied...">{{ old('description', $commissionType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex cursor-pointer">
                            <input type="radio" name="calculation_type" value="fixed" class="peer sr-only" {{ old('calculation_type', $commissionType->calculation_type) === 'fixed' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex cursor-pointer">
                            <input type="radio" name="calculation_type" value="percentage" class="peer sr-only" {{ old('calculation_type', $commissionType->calculation_type) === 'percentage' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                            <span class="text-gray-500 font-semibold" id="rate-prefix">{{ $commissionType->calculation_type === 'percentage' ? '' : '৳' }}</span>
                        </div>
                        <input type="number" name="default_rate" id="default_rate" value="{{ old('default_rate', $commissionType->default_rate) }}"
                            step="0.01" min="0" {{ $commissionType->calculation_type === 'percentage' ? 'max=100' : '' }}
                        ...
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4">
                            <span class="text-gray-500 font-semibold" id="rate-suffix">{{ $commissionType->calculation_type === 'percentage' ? '%' : 'BDT' }}</span>
                        </div>
                    </div>
                    <p class="mt-1 text-xs text-gray-500" id="rate-help">
                        {{ $commissionType->calculation_type === 'percentage' ? 'Percentage of deal value (max 100%)' : 'Fixed amount paid per conversion' }}
                    </p>
                    @error('default_rate')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex items-center gap-3 p-4 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-all">
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-amber-600 focus:ring-amber-500" {{ old('is_active', $commissionType->is_active) ? 'checked' : '' }}>
                        <div>
                    ...
                    <label class="flex items-center gap-3 p-4 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-all">
                        <input type="checkbox" name="is_default" value="1" class="rounded border-gray-300 text-amber-600 focus:ring-amber-500" {{ old('is_default', $commissionType->is_default) ? 'checked' : '' }}>
                        <div>
                ...
                <div class="bg-gray-50 -mx-6 -mb-6 px-6 py-4 mt-6">
                    <div class="flex items-center justify-between text-sm">
                        <div class="flex items-center gap-4 text-gray-500">
                            <span>Created: {{ $commissionType->created_at->format('M d, Y') }}</span>
                            <span>•</span>
                            <span>{{ $commissionType->users->count() }} users assigned</span>
                        </div>
                    </div>
                </div>

                {{-- Submit --}}
                <div class="flex items-center gap-3 pt-4 border-t border-gray-100 -mx-6 px-6">
                    <button type="submit"
                    ...
                    <a href="{{ route('admin.commission-types.users', $commissionType) }}"
                        class="ml-auto inline-flex items-center gap-2 rounded-xl bg-purple-100 px-6 py-3 text-sm font-semibold text-purple-700 hover:bg-purple-200 transition-all">
                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
