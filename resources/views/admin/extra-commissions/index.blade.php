            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-gray-700 to-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider text-white">Status
                        </th>
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-white">Amount
                        </th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 text-white text-xs font-bold shadow-lg shadow-purple-500/30">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <span
                                    class="inline-flex items-center rounded-lg bg-indigo-100 px-2.5 py-1 text-xs font-bold text-indigo-800">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <span
                                    class="text-lg font-bold text-purple-600">৳{{ number_format($commission->amount) }}</span>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex justify-end gap-1">
                                    @if ($commission->status === 'Pending')
                                        <form action="{{ route('admin.extra-commissions.approve', $commission) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit"
                                    ...
                                    @elseif($commission->status === 'Approved')
                                        <form action="{{ route('admin.extra-commissions.mark-paid', $commission) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit"
                                    ...
                                    <form action="{{ route('admin.extra-commissions.destroy', $commission) }}"
                                        method="POST" onsubmit="return confirm('Delete this commission?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
