                    ...
                    <div class="px-5 py-4" x-data="{ showOutcome: false }">
                        <div class="flex justify-between items-start">
                            <a href="{{ route('leads.show', $meeting->lead) }}" class="block group">
                                <p class="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{{ $meeting->lead->client_name }}</p>
                                <p class="text-xs text-gray-500">{{ $meeting->lead->phone_number }}</p>
                            </a>
                            <div class="text-right">
                                <p class="text-sm font-bold text-indigo-600">{{ \Carbon\Carbon::parse($meeting->meeting_time)->format('h:i A') }}</p>
                                @if($meeting->price)
                                    <p class="text-xs font-bold text-emerald-600">৳{{ number_format($meeting->price, 0) }}</p>
                                @endif
                            </div>
                        </div>
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            <span class="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-semibold bg-purple-100 text-purple-700">{{ $meeting->meeting_type }}</span>
                        ...
                        @if($meeting->outcome === 'Pending')
                            <button @click="showOutcome = !showOutcome" class="mt-3 text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
                                Update Status & Outcome →
                            </button>
                        @endif

                        <!-- Outcome Update Form -->
                        <div x-show="showOutcome" x-cloak x-transition class="mt-4 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200">
                            <form action="{{ route('meetings.update-outcome', $meeting) }}" method="POST" class="space-y-3">
                                @csrf
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="space-y-1">
                                        <label class="text-xs font-semibold text-gray-700">Meeting Status</label>
                                        <select name="meeting_status" class="w-full text-sm rounded-lg border-gray-200 bg-white">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <select name="outcome" class="w-full text-sm rounded-lg border-gray-200 bg-white">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="number" name="price" value="{{ $meeting->price }}" step="0.01" class="w-full text-sm rounded-lg border-gray-200 bg-white" placeholder="0.00">
                                </div>
                                <div class="space-y-1">
                                    <label class="text-xs font-semibold text-gray-700">Notes</label>
                                    <textarea name="notes" placeholder="Notes..." rows="2" class="w-full text-sm rounded-lg border-gray-200 bg-white">{{ $meeting->notes }}</textarea>
                                </div>
                                <button type="submit" class="w-full text-sm px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-semibold shadow-lg shadow-indigo-500/30 hover:shadow-xl transition-all">
                                    Save Changes
                                </button>
                            </form>
                        </div>
        ...
        <div class="overflow-hidden rounded-2xl bg-white shadow-lg shadow-gray-200/50 border border-gray-100">
            ...
            <div class="divide-y divide-gray-100 max-h-96 overflow-y-auto">
                @forelse($upcomingMeetings as $meeting)
                    @if($meeting->lead)
                    <a href="{{ route('leads.show', $meeting->lead) }}" class="block px-5 py-4 transition-colors hover:bg-gray-50 group">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="font-semibold text-gray-900 text-sm group-hover:text-blue-600 transition-colors">{{ $meeting->lead->client_name }}</p>
                            ...
                            <div class="text-right">
                                <p class="text-xs font-bold text-blue-600">{{ $meeting->meeting_date->format('M j') }}</p>
                                <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($meeting->meeting_time)->format('h:i A') }}</p>
                                @if($meeting->price)
                                    <p class="text-xs font-bold text-emerald-600 mt-1">৳{{ number_format($meeting->price, 0) }}</p>
                                @endif
                            </div>
                        </div>
                    </a>
    ...
    <div class="overflow-hidden rounded-2xl bg-white shadow-lg shadow-gray-200/50 border border-gray-100 p-6 mb-6">
        <form method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="space-y-2">
                ...
                <select name="meeting_status" class="rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm font-medium transition-all duration-200 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-500/20">
            ...
            <div class="space-y-2">
                ...
                <select name="outcome" class="rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm font-medium transition-all duration-200 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-500/20">
            ...
            <div class="flex gap-2">
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl font-semibold text-sm shadow-lg shadow-indigo-500/30 hover:shadow-xl transition-all">
                    Filter
                </button>
                <a href="{{ route('meetings.index') }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-semibold text-sm hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-500">Meeting Time</th>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-500">Outcome</th>
                    ...
                    <tbody x-data="{ editing: false }" class="bg-white divide-y divide-gray-100">
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-4">
                                <a href="{{ route('leads.show', $meeting->lead) }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
                            ...
                            <td class="px-4 py-4 text-sm">
                                <p class="text-gray-900 font-medium">{{ $meeting->meeting_date->format('M j, Y') }}</p>
                                <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($meeting->meeting_time)->format('h:i A') }}</p>
                            </td>
                            <td class="px-4 py-4">
                                @if($meeting->meeting_status)
                            ...
                            <td class="px-4 py-4 text-sm">
                                @if($meeting->price)
                                    <span class="font-bold text-emerald-600">৳{{ number_format($meeting->price, 0) }}</span>
                                @else
                        ...
                        <tr x-show="editing" x-cloak x-transition class="bg-gray-50">
                            <td colspan="8" class="px-4 py-4">
                                <form action="{{ route('meetings.update', $meeting) }}" method="POST" class="flex flex-wrap gap-4 items-end">
                                    @csrf
                                    @method('PATCH')
                                    <div class="space-y-1">
                                        <label class="text-xs font-semibold text-gray-700">Meeting Status</label>
                                        <select name="meeting_status" class="text-sm rounded-lg border-gray-200 bg-white">
                                            @foreach($meetingStatuses as $key => $status)
                                    ...
                                    <div class="space-y-1">
                                        <label class="text-xs font-semibold text-gray-700">Price</label>
                                        <input type="number" name="price" value="{{ $meeting->price }}" step="0.01" class="w-28 text-sm rounded-lg border-gray-200 bg-white" placeholder="0.00">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-xs font-semibold text-gray-700">Outcome</label>
                                        <select name="outcome" class="text-sm rounded-lg border-gray-200 bg-white">
                                            @foreach($outcomes as $key => $outcome)
                                    ...
                                    <div class="flex-1 space-y-1">
                                        <label class="text-xs font-semibold text-gray-700">Notes</label>
                                        <input type="text" name="notes" value="{{ $meeting->notes }}" class="w-full text-sm rounded-lg border-gray-200 bg-white">
                                    </div>
                                    <div class="flex gap-2">
