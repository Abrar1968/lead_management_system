                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select name="source" id="source"
                                ...
                                <option value="WhatsApp"
                                    {{ old('source', $lead->source) === 'WhatsApp' ? 'selected' : '' }}>WhatsApp
                                </option>
                                <option value="Messenger"
                                    {{ old('source', $lead->source) === 'Messenger' ? 'selected' : '' }}>Messenger
                                </option>
                                <option value="Website"
                                    {{ old('source', $lead->source) === 'Website' ? 'selected' : '' }}>Website</option>
                            </select>
                            @error('source')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="service_interested" id="service_interested"
                                ...
                                required>
                                <option value="Website"
                                    {{ old('service_interested', $lead->service_interested) === 'Website' ? 'selected' : '' }}>
                                    Website</option>
                                <option value="Software"
                                    {{ old('service_interested', $lead->service_interested) === 'Software' ? 'selected' : '' }}>
                                    Software</option>
                                <option value="CRM"
                                    {{ old('service_interested', $lead->service_interested) === 'CRM' ? 'selected' : '' }}>
                                    CRM</option>
                                <option value="Marketing"
                                    {{ old('service_interested', $lead->service_interested) === 'Marketing' ? 'selected' : '' }}>
                                    Marketing</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                ...
                                <option value="Contacted"
                                    {{ old('status', $lead->status) === 'Contacted' ? 'selected' : '' }}>Contacted
                                </option>
                                <option value="Qualified"
                                    {{ old('status', $lead->status) === 'Qualified' ? 'selected' : '' }}>Qualified
                                </option>
                                <option value="Negotiation"
                                    {{ old('status', $lead->status) === 'Negotiation' ? 'selected' : '' }}>Negotiation
                                </option>
                                <option value="Converted"
                                    {{ old('status', $lead->status) === 'Converted' ? 'selected' : '' }}>Converted
                                </option>
                                <option value="Lost"
                                    {{ old('status', $lead->status) === 'Lost' ? 'selected' : '' }}>Lost</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="priority" id="priority"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="Low"
                                    {{ old('priority', $lead->priority) === 'Low' ? 'selected' : '' }}>Low</option>
                                <option value="Medium"
                                    {{ old('priority', $lead->priority) === 'Medium' ? 'selected' : '' }}>Medium
                                </option>
                                <option value="High"
                                    {{ old('priority', $lead->priority) === 'High' ? 'selected' : '' }}>High</option>
                            </select>
                            @error('priority')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                                ...
                                <select name="assigned_to" id="assigned_to"
                                    ...
                                    @foreach ($salesPersons as $person)
                                        <option value="{{ $person->id }}"
                                            {{ old('assigned_to', $lead->assigned_to) == $person->id ? 'selected' : '' }}>
