<x-app-layout>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="relative flex items-start gap-4">
                    ...
                    <div>
                        <p class="text-sm font-medium text-blue-800">
                            ...
                            <a href="{{ route('leads.daily') }}" class="inline-flex items-center gap-1 font-semibold text-blue-600 underline decoration-2 underline-offset-2 transition-colors hover:text-blue-700">
                                Daily View
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                            </a>
                            or
                            <a href="{{ route('leads.monthly') }}" class="inline-flex items-center gap-1 font-semibold text-blue-600 underline decoration-2 underline-offset-2 transition-colors hover:text-blue-700">
                                Monthly View
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                            </a>
            ...
            {{-- Bulk Actions Bar --}}
            <div class="rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 p-4 shadow-lg shadow-indigo-500/30" id="bulkActionsBar" style="display: none;">
                            ...
                            @forelse($leads as $lead)
                                <tr class="group transition-all duration-200 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-indigo-50/50">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-sm font-bold text-white shadow-lg shadow-blue-500/25">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm font-medium text-gray-700">{{ $lead->customer_name ?? 'Unknown' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="flex flex-wrap gap-1.5">
                                            <span class="inline-flex items-center rounded-lg bg-gradient-to-r from-emerald-500 to-green-600 px-2.5 py-1 text-xs font-semibold text-white shadow-sm">
                                                {{ $lead->source }}
                                            </span>
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-sm font-medium text-gray-700">{{ $lead->assignedTo?->name ?? 'Unassigned' }}</span>
                                        </div>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                            ...
                            @empty
                                <tr>
                                    <td colspan="10" class="px-6 py-16 text-center">
                                        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-gray-100 to-gray-200">
                                        ...
                                        <a href="{{ route('leads.create') }}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition-all duration-300 hover:shadow-xl">
                                            <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
                                            </svg>
                                            Create Lead
        ...
        <form id="bulkDeleteForm" action="{{ route('leads.bulk-delete') }}" method="POST" style="display: none;">
            @csrf
            <div id="deleteLeadIds"></div>
        </form>

        <form id="bulkReassignForm" action="{{ route('leads.bulk-reassign') }}" method="POST" style="display: none;">
            @csrf
            <div id="reassignLeadIds"></div>
            <input type="hidden" name="assigned_to" id="reassignUserId">
        </form>

        <form id="bulkStatusForm" action="{{ route('leads.bulk-status') }}" method="POST" style="display: none;">
            @csrf
        ...
        <div id="reassignModal" class="fixed inset-0 z-50 overflow-y-auto hidden" x-data x-transition>
            <div class="flex min-h-screen items-center justify-center p-4">
                <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onclick="closeReassignModal()"></div>
                <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 shadow-2xl transition-all">
                    <div class="flex items-start gap-4">
                        <div class="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 shadow-lg shadow-indigo-500/30">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Choose a team member to reassign the selected leads</p>
                            <div class="mt-5">
                                <label for="reassign_assigned_to" class="block text-sm font-semibold text-gray-700 mb-2">Assign to:</label>
        ...
        <div id="statusModal" class="fixed inset-0 z-50 overflow-y-auto hidden" x-data x-transition>
            <div class="flex min-h-screen items-center justify-center p-4">
                ...
                <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 shadow-2xl transition-all">
                    <div class="flex items-start gap-4">
                        <div class="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 shadow-lg shadow-amber-500/30">
                            <svg class="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Change the status of all selected leads at once</p>
                            <div class="mt-5">
                                <label for="status_select" class="block text-sm font-semibold text-gray-700 mb-2">New Status:</label>
        ...
        <script>
            ...
            function confirmBulkDelete() {
                ...
                if (confirm(`Are you sure you want to delete ${selectedIds.length} lead(s)? This action cannot be undone and will also delete all related contacts, follow-ups, and meetings.`)) {
