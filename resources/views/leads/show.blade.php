                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="overflow-hidden rounded-2xl bg-white shadow-lg shadow-gray-200/50 border border-gray-100 transition-all duration-300 hover:shadow-xl">
                        ...
                        <div class="p-6">
                            <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                <div class="group">
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-gray-400">Customer Name</dt>
                                    <dd class="mt-2 text-base font-semibold text-gray-900">{{ $lead->customer_name ?? 'Not provided' }}</dd>
                                </div>
                                <div class="group">
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-gray-400">Phone Number</dt>
                                    <dd class="mt-2">
                                        <a href="tel:{{ $lead->phone_number }}" class="inline-flex items-center gap-2 text-base font-semibold text-indigo-600 hover:text-indigo-500 transition-colors">
                                ...
                                <div class="group">
                                    ...
                                    <dd class="mt-2">
                                        @if($lead->email)
                                            <a href="mailto:{{ $lead->email }}" class="inline-flex items-center gap-2 text-base font-semibold text-indigo-600 hover:text-indigo-500 transition-colors">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="group">
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-gray-400">Source</dt>
                                    <dd class="mt-2">
                                        <span class="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-3 py-1.5 text-xs font-bold text-white shadow-lg shadow-cyan-500/30">
                                            {{ $lead->source }}
                                        </span>
                                    </dd>
                                </div>
                                ...
                                <div class="group">
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-gray-400">Assigned To</dt>
                                    <dd class="mt-2 flex items-center gap-2">
                                        @if($lead->assignedTo)
                                            <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 text-xs font-bold text-white shadow-lg shadow-indigo-500/30">
                                                {{ substr($lead->assignedTo->name, 0, 1) }}
                                            </span>
                                            <span class="text-base font-semibold text-gray-900">{{ $lead->assignedTo->name }}</span>
                                        @else
                            ...
                            @if($lead->initial_remarks)
                                <div class="mt-6 rounded-xl bg-gray-50 p-4 border border-gray-100">
                                    <dt class="text-xs font-semibold uppercase tracking-wider text-gray-400">Initial Remarks</dt>
                                    <dd class="mt-2 text-sm text-gray-700 whitespace-pre-line leading-relaxed">{{ $lead->initial_remarks }}</dd>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden rounded-2xl bg-white shadow-lg shadow-gray-200/50 border border-gray-100 transition-all duration-300 hover:shadow-xl">
                        <div class="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4 flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center rounded-xl bg-white/20 px-3 py-1.5 text-xs font-bold text-white">
                            ...
                            @else
                                <div class="space-y-4">
                                    @foreach($lead->contacts as $contact)
                                        <div class="group flex items-start gap-4 rounded-xl border border-gray-100 p-4 transition-all duration-300 hover:border-gray-200 hover:shadow-md">
                                            <div class="flex-shrink-0">
                                                <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl shadow-lg
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center justify-between">
                                                    <span class="text-sm font-bold text-gray-900">{{ $contact->response_status }}</span>
                                                ...
                                                @if($contact->notes)
                                                    <p class="mt-2 text-sm text-gray-600 leading-relaxed">{{ $contact->notes }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            @else
                                <div class="space-y-3">
                                    @foreach($lead->followUps as $followUp)
                                        <div class="flex items-center justify-between rounded-xl border p-4 transition-all duration-300 hover:shadow-md
                                            ...
                                            <span class="inline-flex items-center rounded-xl px-3 py-1.5 text-xs font-bold shadow-lg
                    ...
                    <div class="overflow-hidden rounded-2xl bg-white shadow-lg shadow-gray-200/50 border border-gray-100 transition-all duration-300 hover:shadow-xl">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between gap-3">
                                <span class="inline-flex items-center rounded-xl px-4 py-2 text-sm font-bold shadow-lg
                                    @switch($lead->status)
                            ...
                            @if($lead->conversion)
                                <div class="mt-6 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 p-4 border border-emerald-200 shadow-lg shadow-emerald-500/10">
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="text-sm font-bold text-emerald-900">৳{{ number_format($lead->conversion->deal_value) }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-sm text-emerald-700">Commission</span>
                                            <span class="text-sm font-bold text-emerald-900">৳{{ number_format($lead->conversion->commission_amount) }}</span>
                                        </div>
                        ...
                        <div class="p-6">
                            <dl class="space-y-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <dd class="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-100 text-sm font-bold text-emerald-700">{{ $lead->contacts->count() }}</dd>
                                </div>
                                <div class="flex justify-between items-center">
                                    <dt class="text-sm font-medium text-gray-500">Follow-ups</dt>
                                    <dd class="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-100 text-sm font-bold text-amber-700">{{ $lead->followUps->count() }}</dd>
                                </div>
                                <div class="flex justify-between items-center">
                                    <dt class="text-sm font-medium text-gray-500">Meetings</dt>
                                    <dd class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-100 text-sm font-bold text-blue-700">{{ $lead->meetings->count() }}</dd>
                                </div>
                                <div class="pt-4 border-t border-gray-100 space-y-3">
                                    <div class="flex justify-between">
                                        <dt class="text-sm text-gray-500">Created</dt>
                                        <dd class="text-sm font-medium text-gray-900">{{ $lead->created_at->diffForHumans() }}</dd>
                                    </div>
                                    <div class="flex justify-between">
