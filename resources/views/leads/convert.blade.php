    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <div
                    class="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/30">
        ...
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 lg:grid-cols-5">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div
                        ...
                        <div class="divide-y divide-gray-100 p-4">
                            ...
                            <div class="flex justify-between py-3">
                                ...
                                <span
                                    class="inline-flex items-center rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 px-2.5 py-1 text-xs font-bold text-white shadow-lg shadow-purple-500/30">{{ $lead->service->name ?? 'N/A' }}</span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-sm font-medium text-gray-500">Source</span>
                                <span
                                    class="inline-flex items-center rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 px-2.5 py-1 text-xs font-bold text-white shadow-lg shadow-cyan-500/30">{{ $lead->source }}</span>
                            </div>
                            <div class="flex justify-between py-3">
                                <span class="text-sm font-medium text-gray-500">Lead Date</span>
                                <span
                                    class="text-sm font-bold text-gray-900">{{ $lead->lead_date->format('M d, Y') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-purple-700 shadow-lg shadow-indigo-500/30 transition-all duration-300 hover:shadow-xl">
                        <div class="p-6">
                            ...
                            <div class="text-white/80">
                                @if ($user->commission_type === 'fixed')
                                    ...
                                    <p class="text-3xl font-bold text-white mt-1">
                                        ৳{{ number_format($user->default_commission_rate) }}</p>
                                    <p class="text-xs text-white/60 mt-1">per conversion</p>
                                @else
                                    <p class="text-sm">Percentage Rate</p>
                                    <p class="text-3xl font-bold text-white mt-1">{{ $user->default_commission_rate }}%
                                    </p>
                    ...
                    <div
                        ...
                        <form action="{{ route('conversions.store', $lead) }}" method="POST" class="p-6"
                            ...
                            <div class="mb-6 space-y-2">
                                <label for="deal_value" class="block text-sm font-semibold text-gray-700">Deal Value
                                    (BDT) <span class="text-red-500">*</span></label>
                                <div class="relative">
                                    <span
                                        class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-semibold">৳</span>
                                    <input type="number" name="deal_value" id="deal_value" x-model="dealValue"
                                        value="{{ old('deal_value') }}" required min="0" step="0.01"
                                        placeholder="0.00"
                            ...
                            {{-- Conversion Date --}}
                            <div class="mb-6 space-y-2">
                                <label for="conversion_date"
                                    class="block text-sm font-semibold text-gray-700">Conversion Date <span
                                        class="text-red-500">*</span></label>
                                <input type="date" name="conversion_date" id="conversion_date"
                                    value="{{ old('conversion_date', now()->format('Y-m-d')) }}" required
                            ...
                            {{-- Notes --}}
                            <div class="mb-6 space-y-2">
                                <label for="notes" class="block text-sm font-semibold text-gray-700">Notes
                                    (Optional)</label>
                            ...
                            {{-- Commission Preview --}}
                            <div
                                class="mb-8 rounded-2xl bg-gradient-to-r from-emerald-50 to-green-50 p-5 border border-emerald-200 shadow-lg shadow-emerald-500/10">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="text-right">
                                        <p class="text-3xl font-bold text-emerald-700"
                                            x-text="formatCurrency(commission)">৳0</p>
                                    </div>
                                </div>
                            </div>
