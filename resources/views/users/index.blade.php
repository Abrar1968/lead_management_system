                                        ...
                                        <div class="flex justify-end gap-2">
                                            ...
                                            @if ($user->id !== auth()->id())
                                                <button type="button"
                                                    ...
                                                    @click="showDeleteModal = true; deleteUserId = {{ $user->id }}; deleteUserName = '{{ $user->name }}'; leadsCount = {{ $user->leads_count }}; deleteUrl = '{{ route('users.destroy', $user) }}'">
                                                    Delete
                                                </button>
                                            @endif
                                        </div>
                ...
                <div x-show="showDeleteModal"
                                    ...
                                    <template x-if="leadsCount > 0">
                                        <div class="mt-4 space-y-3 border-t pt-4">
                                            <p class="text-sm font-medium text-amber-700">
                                                ⚠️ This user has <span x-text="leadsCount"></span> assigned lead(s). What should happen to them?
                                            </p>
                                            <div class="space-y-2">
                                                <label class="flex items-center gap-2">
                                                    <input type="radio" name="lead_action_modal" value="reassign" x-model="leadAction"
                                                           class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                                    <span class="text-sm text-gray-700">Reassign all leads to Admin</span>
                                                </label>
                                                <label class="flex items-center gap-2">
                                                    <input type="radio" name="lead_action_modal" value="delete" x-model="leadAction"
                                                           class="h-4 w-4 border-gray-300 text-red-600 focus:ring-red-500">
                                                    <span class="text-sm text-red-700">Delete all leads permanently</span>
                                                </label>
                                            </div>
                                        </div>
                                    </template>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                        <form :action="deleteUrl" method="POST" class="inline">
                            ...
                            <button type="submit"
                                    :disabled="leadsCount > 0 && (leadAction === 'cancel' || leadAction === '')"
                        ...
                        <button type="button"
                                @click="showDeleteModal = false; leadAction = 'cancel'"
