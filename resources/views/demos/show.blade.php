                        ...
                        <div class="p-6 space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    @endphp
                                    <p class="mt-1">
                                        <span class="px-3 py-1 rounded-full text-sm font-semibold {{ $statusColors[$demo->status] ?? 'bg-gray-100 text-gray-800' }}">
                                            {{ $demo->status }}
                                        </span>
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <p class="mt-1">
                                        @if($demo->type === 'Online')
                                            <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-semibold bg-cyan-100 text-cyan-800">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-gray-900 font-semibold">{{ $demo->demo_date->format('F d, Y') }}</p>
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 uppercase">Time</label>
                                    <p class="mt-1 text-gray-900 font-semibold">{{ $demo->demo_time ? $demo->demo_time->format('h:i A') : 'Not set' }}</p>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    @foreach($dynamicFields as $field)
                                        ...
                                        <div class="{{ $field->type === 'image' ? 'md:col-span-2' : '' }}">
                                            <label class="text-xs font-semibold text-gray-500 uppercase">{{ $field->label }}</label>
                                            @if($field->type === 'image')
                                                @if($value)
                                                    <div class="mt-2">
                ...
                <div class="space-y-6">
                        ...
                        <div class="p-6">
                            @if($demo->lead)
                                <div class="flex items-center gap-4 mb-4">
                                    <div class="h-14 w-14 rounded-full bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-emerald-500/30">
                                        {{ strtoupper(substr($demo->lead->client_name ?? $demo->lead->phone_number, 0, 1)) }}
                                    </div>
                                    <div>
                                        <h4 class="font-bold text-gray-900">{{ $demo->lead->client_name ?? 'N/A' }}</h4>
                                ...
                                <div class="space-y-2 text-sm">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium text-gray-900">{{ $demo->lead->service_interested }}</span>
                                    </div>
                                </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 rounded-full bg-gradient-to-br from-gray-500 to-gray-600 flex items-center justify-center text-white text-lg font-bold">
                                    {{ strtoupper(substr($demo->createdBy->name ?? 'U', 0, 1)) }}
                                </div>
                                <div>
                                    <h4 class="font-bold text-gray-900">{{ $demo->createdBy->name ?? 'Unknown' }}</h4>
                                    <p class="text-sm text-gray-500">{{ $demo->created_at->format('M d, Y h:i A') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    @if(auth()->user()->isAdmin())
                        <div class="bg-red-50 rounded-2xl border border-red-200 p-6">
                            ...
                            <p class="text-sm text-red-600 mb-4">Permanently delete this demo. This action cannot be undone.</p>
