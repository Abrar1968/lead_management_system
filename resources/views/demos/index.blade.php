                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Demo</th>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Lead</th>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Schedule</th>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Type</th>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Status</th>
                                <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-600">Created By</th>
                            ...
                            @forelse($demos as $demo)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        ...
                                        @if($demo->description)
                                            <div class="text-sm text-gray-500 truncate max-w-xs">{{ Str::limit($demo->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($demo->lead)
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="text-sm font-medium text-gray-900">{{ $demo->lead->client_name ?? 'N/A' }}</div>
                                                    <div class="text-xs text-gray-500">{{ $demo->lead->phone_number }}</div>
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $demo->demo_date->format('M d, Y') }}</div>
                                        @if($demo->demo_time)
                                            <div class="text-xs text-gray-500">{{ $demo->demo_time->format('h:i A') }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($demo->type === 'Online')
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-cyan-100 text-cyan-800">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusColors[$demo->status] ?? 'bg-gray-100 text-gray-800' }}">
                                            {{ $demo->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
