            ...
            <form action="{{ route('demos.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="lead_id" name="lead_id"
                                ...
                                @foreach($leads as $lead)
                                    <option value="{{ $lead->id }}" {{ old('lead_id', $selectedLead) == $lead->id ? 'selected' : '' }}>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" id="demo_date" name="demo_date" value="{{ old('demo_date', date('Y-m-d')) }}" required
                        ...
                        <div>
                            ...
                            <select id="type" name="type" required
                                    class="w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                    x-data="{ selected: '{{ old('type', 'Online') }}' }"
                                    x-model="selected">
                ...
                <div class="space-y-4" x-data="{ type: '{{ old('type', 'Online') }}' }">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <!-- Meeting Link (Online) -->
                        <div x-show="document.getElementById('type').value === 'Online' || '{{ old('type', 'Online') }}' === 'Online'">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($dynamicFields as $field)
                                <div class="{{ $field->type === 'text' && strlen($field->label) > 30 ? 'md:col-span-2' : '' }}">
                                    ...
                                    @elseif($field->type === 'link')
                                        <input type="url" id="dynamic_{{ $field->name }}" name="dynamic_{{ $field->name }}"
                                               value="{{ old('dynamic_' . $field->name) }}" {{ $field->required ? 'required' : '' }}
                                               class="w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                               placeholder="https://...">
                                    @else
